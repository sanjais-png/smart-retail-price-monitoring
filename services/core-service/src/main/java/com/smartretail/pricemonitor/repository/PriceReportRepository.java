...
public interface PriceReportRepository extends JpaRepository<PriceReport, Long> {
    ...
    @Query("SELECT AVG(pr.price) FROM PriceReport pr WHERE pr.commodity.id = :commodityId AND LOWER(pr.market.city) = LOWER(:city) AND pr.status = 'VERIFIED'")
    Optional<BigDecimal> findAverageVerifiedPriceByCity(@Param("commodityId") Long commodityId, @Param("city") String city);

    @Query("SELECT AVG(pr.price) FROM PriceReport pr WHERE pr.commodity.id = :commodityId AND pr.status = 'VERIFIED'")
    Optional<BigDecimal> findAverageVerifiedPriceNationwide(@Param("commodityId") Long commodityId);
